...
const scrollThreshold = 0.001; // Define la cantidad de píxeles necesarios para desencadenar la actualización de tiempo de reproducción
...
window.addEventListener('wheel', (event) => {
  ...
  if (event.deltaY < -scrollThreshold) {
    video.currentTime -= 0.1; // Retrocede el video en un segundo
  } else if (event.deltaY > scrollThreshold) {
    video.currentTime += 0.1; // Avanza el video en un segundo
  }
});